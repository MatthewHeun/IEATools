  ...
  .iea_df %>% 
    ...
    dplyr::mutate(
      ...
      !!as.name(flow_aggregation_point) := dplyr::case_when(
        ...
        !!as.name(ledger_side) == supply & !!as.name(flow) == transfers ~ paste0(tfc_compare, " (", transfers, ")"),
        !!as.name(ledger_side) == supply & !!as.name(flow) == statistical_differences ~ paste0(tfc_compare, " (", statistical_differences, ")"),
        !!as.name(ledger_side) == supply & !!as.name(flow) == losses ~ paste0(tfc_compare, " (", losses, ")"),

...
#' @param text a connection to \code{iea_file}.
...
#'
#' @return a data frame containing the as-read IEA data file
#' 
#' @export
iea_df <- function(iea_file = NULL, text = NULL, expected_1st_line_start = ",,TIME", expected_2nd_line = "COUNTRY,FLOW,PRODUCT"){
  assertthat::assert_that(xor(is.null(iea_file), is.null(text)), msg = "need to supply one but not both of iea_file and text arguments to iea_df")
  if (!is.null(iea_file)) {
  ...
  # because it simply replaces the first 3 items with appropriate values from the 2nd line.

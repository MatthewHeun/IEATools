...
The `IEAData` package has functions to address each of these issues.
...
The default arguments are consistent throughout the `IEAData` package.
...
`Ledger.side`, `Flow.aggregation.point`, `Energy.type`, and `Units`.
...
The `tidy_iea_data()` function converts to a tidy format.
By default, `tidy_iea_data()` removes zeroes from the data frame,
...
At this point, the IEA data are ready for analyses.

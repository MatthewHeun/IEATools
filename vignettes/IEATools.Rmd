...
Data are grouped internal to `calc_tidy_iea_df_balances()`
using the `grouping_vars` argument whose default value is
`c("Method", "Last.stage", "Country", "Year", "Energy.type", "Unit", "Product")`.
...
country-level energy balance calculations and product-level balance calculations.
...
Notice that the country-level IEA data are not quite energy balanced.
...
Tidy_IEA_df %>% 
  ...
  calc_tidy_iea_df_balances() %>% 
  glimpse()
# Country level data are also balanced
Tidy_IEA_df %>% 
  fix_tidy_iea_df_balances() %>% 
  calc_tidy_iea_df_balances(grouping_vars = c("Method", "Last.stage", "Country",
                                              "Year", "Energy.type", "Unit")) %>% 
...
The next step could be to prepare for analysis within the
physical supply-use table (PSUT) framework.
See http://prep_psut.html for details.

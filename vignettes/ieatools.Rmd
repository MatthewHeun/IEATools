...
### Load a `.csv` file to a tidy IEA data frame

For simplicity, 
the above functions are rolled into `load_tidy_iea_df()`.
`load_tidy_iea_df()` loads the sample data bundled with the package and 
...
Tidy_IEA_df %>% 
  ...
  calc_tidy_iea_df_balances() %>% 
  glimpse()
# Product-level balances within each country
Tidy_IEA_df %>% 
  group_by(Country, Year, Energy.type, Product) %>% 
  calc_tidy_iea_df_balances() %>% 
  glimpse()

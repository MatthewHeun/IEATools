...
`tp_sinks_sources()` returns `grouping_vars` and the `Flow` column.
...
data.frame(Flow = c("Automobiles", "Automobiles", "Furnaces"),
  ...
  tp_sinks_sources(type = "sources", grouping_vars = "Country")
```
...
For details, see the [Prep PSUT](http://prep_psut.html) vignette.

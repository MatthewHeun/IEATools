...
test_that("Aggregating South Africa and Ghana works as intended", {
  ...
  manual_aggregation <- tidy_GHA_ZAF_df %>%
    dplyr::group_by(Year, LedgerSide, Flow.aggregation.point, Flow, Product, Unit) %>% 
  ...
  comparing <- aggregated_regions %>%
    dplyr::full_join(manual_aggregation, 
                     by = dplyr::join_by(Year, LedgerSide, Flow.aggregation.point, Flow, Product, Unit)) %>% 
    dplyr::mutate(
  ...
  manual_aggregation_excl_ie <- tidy_GHA_ZAF_df %>% 
    ...
    dplyr::group_by(Method, LastStage, EnergyType, Year, LedgerSide, Flow.aggregation.point, Flow, Product, Unit) %>%
    dplyr::summarise(E.dot.aggregated = sum(E.dot), .groups = "drop") %>% 
    dplyr::mutate(
      Country = "GHAZAF"
    )
  ...
  manual_aggregation_ie <- tidy_GHA_ZAF_df %>%
    ...
    dplyr::filter(! (stringr::str_detect(Flow, IEATools::tpes_flows$exports_to_world_marine_bunkers) |
                       stringr::str_detect(Flow, IEATools::tpes_flows$exports_to_world_aviation_bunkers))) %>%
    dplyr::group_by(Method, EnergyType, LastStage, Year, LedgerSide, Flow.aggregation.point, Flow, Product, Unit) %>%
    dplyr::summarise(E.dot.aggregated = sum(E.dot), .groups = "drop") %>%
    remove_suffix_specifications(col = IEATools::iea_cols$flow, unsuffixed_col = IEATools::iea_cols$flow) %>%
  ...
  comparing <- aggregated_regions %>%
    dplyr::full_join(manual_aggregation, by = c("Country", "Method", "EnergyType", "LastStage", "Year", "LedgerSide", "Flow.aggregation.point", "Flow", "Product", "Unit")) %>%
    dplyr::mutate(
      is_equal = E.dot.aggregated == E.dot
    )
...
test_that("Aggregating ZAF and MGC, and GHA and EGC, works as intended", {
  ...
  manual_aggregation <- tidy_GHA_ZAF_EGC_MGC_df %>%
    ...
    dplyr::group_by(Country, Year, LedgerSide, Flow.aggregation.point, Flow, Product) %>%
    dplyr::summarise(E.dot.aggregated = sum(E.dot), .groups = "drop")
  
  comparing <- aggregated_regions %>%
    dplyr::full_join(manual_aggregation, 
                     by = dplyr::join_by(Country, Year, LedgerSide, Flow.aggregation.point, Flow, Product)) %>%
    dplyr::mutate(
  ...
  manual_aggregation_excl_ie <- tidy_GHA_ZAF_EGC_MGC_df %>%
    ...
    dplyr::group_by(Country, Method, EnergyType, LastStage, Year, LedgerSide, Flow.aggregation.point, Flow, Product, Unit) %>%
    dplyr::summarise(E.dot.aggregated = sum(E.dot), .groups = "drop")
  ...
  comparing <- aggregated_regions %>%
    dplyr::full_join(manual_aggregation, 
                     by = dplyr::join_by(Country, Method, EnergyType, LastStage, Year,
                                         LedgerSide, Flow.aggregation.point, Flow, Product, Unit)) %>%
    dplyr::mutate(
      difference = E.dot.aggregated - E.dot
    )
